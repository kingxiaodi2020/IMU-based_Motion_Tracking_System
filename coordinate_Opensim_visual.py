...
"""
from matplot import draw_quaternion
import visualize
...
input_file = r"inv_transformed_test3_vis"  # Replace with your input .sto file path
output_file = "gif_exp3.sto"  # Replace with your desired output .sto file path
IMU_INSTALL_ROTATION = False  # IMU SETUP Set to True if using installation rotation matrix
...
#     [0,  1,  0],  # IMU X → OpenSim Y
#     [0,  0,  -1],  # IMU Y → OpenSim Z
#     [-1,  0,  0]   # IMU Z → OpenSim X
# ])

R_install_scapula = np.array([
    [0, 0, -1],  # IMU -Z → OpenSim X       Anboshi
    [1, 0, 0],   # IMU X → OpenSim Y
    [0, -1, 0]   # IMU -Y → OpenSim Z
])


# R_install_humerus = np.array([
#     [ 0, -1,  0],  # IMU X → OpenSim -Y
#     [ 1,  0,  0],  # IMU Y → OpenSim Z
#     [ 0,  0,  1]   # IMU Z → OpenSim X
# ])

R_install_humerus = np.array([
    [0, 1, 0],  # IMU Y → OpenSim X     Anboshi
    [-1, 0, 0], # IMU -X → OpenSim Y
    [0, 0, 1]   # IMU Z → OpenSim Z
])

# R_install_thorax = np.array([
#     [ 0,  0,  -1],  # IMU X → OpenSim Z
#     [ 0,  1,  0],   # IMU Y → OpenSim Y
#     [ 1,  0,  0]    # IMU Z → OpenSim X
# ])
...
def pandas_to_numpy(data):
    ...
    return [np.array([float(x) for x in quaternion_str.split(',')]) for quaternion_str in data]
...
def process_sto(input_file, output_file):
    ...
    # draw_quaternion(pandas_to_numpy(scapula_data), time_data, name="Marker_scapula2")

    # 对肱骨IMU数据进行转换
    humerus_data = data['humerus_imu']
    # get numpy array from humerus_data
    humerus_np = [np.array([float(x) for x in quaternion_str.split(',')]) for quaternion_str in humerus_data]
    # draw_quaternion(humerus_np)
    ...
    for quaternion_str in humerus_data:
        ...
        # q_global = q_BNO085 #transform_quaternion(q_BNO085, R_install_humerus, np.eye(3))
        transformed_humerus_data.append(','.join([f"{x:.6f}" for x in q_transformed]))
        # global_humerus_data.append(','.join([f"{x:.6f}" for x in q_global]))
    data['humerus_imu'] = transformed_humerus_data
    draw_quaternion(pandas_to_numpy(humerus_data), time_data, name="humerus2")
    # draw_quaternion(pandas_to_numpy(global_humerus_data), name="GLOBAL_humerus")
    ...
    # draw_quaternion(pandas_to_numpy(thorax_data), time_data, name="Marker_thorax2")
